...
public class Message
{
	...
	public static Message createMessage(int receiverId, int senderId, int msgType, String msgText)
	{
		...
		String senderName = PlayerDBUtils.getNameFromUid(senderId);
	...
	public static Message createMatchReqMessage(int receiverId, Match match)
	{
		Message tmpMsg = new Message();
		int tmpId = DBUtils.getMaxTableId("WM_WIADOMOSCI") + 1;
		tmpMsg.setId(tmpId);
		...
		if(match.getHomeTeamId() == receiverId)
		{
			...
			String opponentName = TeamDBUtils.getTeamNameFromId(match.getAwayTeamId());
			nextMatchMsg += "#" + opponentName;
		}
		else
		{
			nextMatchMsg += "#" + match.getHomeTeamId();
			String opponentName = TeamDBUtils.getTeamNameFromId(match.getHomeTeamId());
			nextMatchMsg += "#" + opponentName;
		}
		String leagueName = LeagueDBUtils.getLeagueNameFromId(match.getLeagueId());
	...
	public static Message createMatchPropMessage(int receiverId, Match match, int propCnt, String terms)
	{
		Message tmpMsg = new Message();
		int tmpId = DBUtils.getMaxTableId("WM_WIADOMOSCI") + 1;
		tmpMsg.setId(tmpId);
	...
	public static Message createMatchAcceptMessage(int receiverId, Match match, String term)
	{
		Message tmpMsg = new Message();
		int tmpId = DBUtils.getMaxTableId("WM_WIADOMOSCI") + 1;
		tmpMsg.setId(tmpId);
